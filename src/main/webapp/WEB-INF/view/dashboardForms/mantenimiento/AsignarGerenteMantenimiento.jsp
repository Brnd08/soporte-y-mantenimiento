...
<%
    ...
    List<Usuario> gerentesMantenimientoDisponibles = (List<Usuario>) session.getAttribute("gerentes-mantenimiento");
    System.out.println(gerentesMantenimientoDisponibles);


    String errorAsignacionGerenteSoporte = (String) session.getAttribute("errorAsignacionGerenteSoporte");
    System.out.println(errorAsignacionGerenteSoporte);

    String exitoAsignacionGerenteSoporte = (String) session.getAttribute("exitoAsignacionGerenteSoporte");
    ...
    <form method="post" action="/asignar-reporte"
        ...
        <div>
            <div class="form-check">
                <input class="form-check-input" type="radio" name="usuarioAsignado" id="autoAsignarseGerenteMantenimiento" checked>
                <label class="form-check-label" for="autoAsignarseGerenteMantenimiento">Autoasignarse Reporte</label>
            </div>
            <div class="form-check">
                <input class="form-check-input" type="radio" name="usuarioAsignado" id="asignarOtroGerenteMantenimiento">
                <label class="form-check-label" for="asignarOtroGerenteMantenimiento">Asignar a otro gerente Mantenimiento</label>
            </div>
        </div>
        <%-- ---------------------------------- CHECKBOX USUARIO A ASIGNAR ----------------------------------------------------%>

        <div id="divGerenteMantenimientoAsignar" class="form-floating mb-3" style="display: none">
            <select name="idUsuario" id="gerenteMantenimientoAsignado" class="form-control"
        ...
        <script>
            const checkboxAsignarOtroGerenteMantenimiento = document.getElementById("asignarOtroGerenteMantenimiento");
            const checkboxAutoAsignarGerenteMantenimiento = document.getElementById("autoAsignarseGerenteMantenimiento");
            const divGerenteAsignar = document.getElementById("divGerenteMantenimientoAsignar");
            const gerenteMantenimientoAsignado = document.getElementById("gerenteMantenimientoAsignado");

            checkboxAsignarOtroGerenteMantenimiento.addEventListener("change", ev => {
                divGerenteAsignar.style.display = 'inherit';
                gerenteMantenimientoAsignado.disabled = false;
            })
            checkboxAutoAsignarGerenteMantenimiento.addEventListener("change", ev => {
                divGerenteAsignar.style.display = 'none';
